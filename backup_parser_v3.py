...
# 3. ===SETTING TAB MENU TAMPILAN GUI===
    ...
    def create_customer_tab(self):
        """Create Customer information tab"""
        # Configure grid weights
        self.customer_frame.grid_rowconfigure(0, weight=0)  # Form frame
        self.customer_frame.grid_rowconfigure(1, weight=1)  # Customer list (expandable)
        self.customer_frame.grid_rowconfigure(2, weight=0)  # Current customer display
        self.customer_frame.grid_columnconfigure(0, weight=1)
        
        # ===== FORM FRAME =====
        form_frame = ttk.LabelFrame(self.customer_frame, text="Customer Information Form", padding=20)
        ...
        # ===== CUSTOMER LIST FRAME =====
        list_frame = ttk.LabelFrame(self.customer_frame, text="Customer Database", padding=10)
        list_frame.grid(row=1, column=0, padx=10, pady=10, sticky='nsew')
        list_frame.grid_rowconfigure(0, weight=1)
        list_frame.grid_columnconfigure(0, weight=1)

        # Treeview - HANYA 4 kolom yang ditampilkan (ID dan Created disembunyikan)
        ...
        # Column configuration - lebih lebar karena hanya 4 kolom
        ...
        # Width lebih besar untuk 4 kolom (total space dibagi lebih sedikit kolom)
        ...
        current_frame = ttk.LabelFrame(self.customer_frame, text="Current Active Customer", padding=15)
        current_frame.grid(row=2, column=0, padx=10, pady=10, sticky='ew')
        ...
        self.current_customer_label.pack(fill=tk.BOTH, expand=True)
        ...
        for customer in customers:
            ...
            item_id = self.customer_tree.insert('', tk.END, 
                values=(
                    code,                                      # Customer Code
                    name,                                      # Customer Name
                    sales_code if sales_code else '-',        # Sales Code
                    sales_name if sales_name else '-'         # Sales Name
                ),
                tags=(str(customer_id),)  # Simpan ID di tags untuk referensi internal
            )
    ...
    def edit_selected_customer(self):
        ...
        if hasattr(self, 'customer_status_label'):
            self.customer_status_label.configure(
                text=f"📝 Editing: [{values[0]}] {values[1]}",  # FIX: index 0 dan 1
                fg='#3498db'
            )
        
        # Visual feedback
        self.log_multi_serial(f"Editing customer: [{values[0]}] {values[1]}")  # FIX: index 0 dan 1
        ...
        messagebox.showinfo(
            ...
            f"Code: {values[0]}\n"      # FIX: index 0
            f"Name: {values[1]}\n\n"    # FIX: index 1
            f"✏️ Please modify the data and click 'Save to Database' to update."
        )
    ...
    def parse_hl7(self, hl7_text):
        ...
        for line_num, line in enumerate(lines):
            ...
            # OBX SEGMENT (Observation Results)
                ...
                # ✅ PARSE OBX for URIT and Standard devices
                try:
                    if len(segments) > 5:
                        # Field 3: Observation Identifier
                        obs_id = segments[3].split('^') if len(segments) > 3 else []
                        
                        # Determine test name and value based on format
                        if len(obs_id) > 1:
                        ...
                        # Add result if test name and value exist
