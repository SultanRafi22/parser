...
# 1. ===SETTING INISIALISASI===
    def __init__(self, root):
        self.root = root
        self.root.title("Parser MLLP HL7 - LIMS Simulation")
        ...
        self.customer_file = "current_customer.json"
        ...
        # Bind window state events (alternative method)
...
# ===== MENGATUR TRAY ICON ======
    def create_tray_icon(self):
        ...
        width = 64
        height = 64
        color1 = (0, 120, 212)  # Blue color
        color2 = (255, 255, 255)  # White color
        
        image = Image.new('RGB', (width, height), color1)
        dc = ImageDraw.Draw(image)
        
        # Draw a simple pattern (optional)
        dc.rectangle(
        ...
        # Create menu items
        menu = (
            item('Show Window', self.show_window, default=True),  # Default action for double-click
            item('Hide to Tray', self.hide_window),
            pystray.Menu.SEPARATOR,
            item('Exit Application', self.quit_from_tray)
        )
        
        # Create the tray icon with double-click handler
        self.tray_icon = pystray.Icon(
            "HL7_Parser", 
            image, 
            "HL7 Parser - LIMS", 
            menu,
            on_activate=self.show_window  # Double-click shows window
        )
    ...
    def _show_window(self):
        """Internal method to show window (must run in main thread)"""
        self.root.deiconify()
        ...
        self.log_multi_serial("Application restored from system tray")
        
    def hide_window(self, icon=None, item=None):
        """Hide the main window to tray"""
        self.root.withdraw()
        self.log_multi_serial("Application hidden to system tray")
        
    def quit_from_tray(self, icon=None, item=None):
        """Quit application from tray"""
        self.log_multi_serial("Exit requested from system tray")
            
        if icon:
            icon.stop()
            
            # Close application directly (no confirmation from tray)
        self.root.after(0, self._force_exit)
        
    def on_closing(self):
        """Handle window close button (X) - EXIT application"""
    ...
    def on_window_state_change(self, event):
        ...
        if event.type == '18':  # MapNotify event
            pass
        elif event.type == '19':  # UnmapNotify event  
            # Window is being unmapped (minimized or hidden)
            if self.root.state() == 'iconic':
                # Window was minimized - hide to tray
                self.root.after(10, self.hide_window_to_tray)
        ...
        if hasattr(self, 'tray_icon'):
            try:
                self.tray_icon.notify(
                    "Minimized to Tray",
                    "HL7 Parser is running in the background.\nDouble-click tray icon to restore."
                )
            except:
                pass  # Notification not supported

    def check_window_state(self):
        """Periodically check if window is minimized"""
        try:
            if self.root.state() == 'iconic':
                # Window is minimized - hide to tray
                self.hide_window_to_tray_silent()
        except:
            pass
    ...
    def hide_window_to_tray_silent(self):
        """Hide window to tray without notification spam"""
        current_time = time.time()
        
        # Prevent multiple rapid hides (debounce)
        if not hasattr(self, '_last_minimize_time'):
            self._last_minimize_time = 0
        
        if current_time - self._last_minimize_time > 1:  # 1 second debounce
            self._last_minimize_time = current_time
            self.root.withdraw()
            
            # Log the action
            self.log_multi_serial("Application minimized to system tray")
            ...
            if hasattr(self, 'tray_icon'):
                try:
                    ...
                    pass
# ===== BATAS PENGATURAN UNTUK TRAY ICON ======

    def load_device_labels(self):
        """Load label alat yang sudah disimpan dari file JSON"""
        if os.path.exists(self.device_labels_file):
            try:
                with open(self.device_labels_file, "r") as f:
                    loaded_data = json.load(f)
                
                if "socket" in loaded_data and "serial" in loaded_data:
                    for device_type in ["socket", "serial"]:
                        for identifier, value in loaded_data[device_type].items():
                            if isinstance(value, str):
                                loaded_data[device_type][identifier] = {
                                    "label": value,
                                    "serial_number": "",
                                    "device_type": ""
                                }
                    ...
                    self.device_labels = loaded_data
                else:
                    self.device_labels = {"socket": {}, "serial": {}}
    ...
    def save_device_labels(self):
        """Menyimpan label alat ke file JSON"""
        try:
    ...
    def load_app_configuration(self):
        """Load konfigurasi data yang disimpan di aplikasi"""
        if os.path.exists(self.config_file):
            try:
                ...
                print("Configuration loaded from last session") 
                return True
    ...
    def save_app_configuration(self):
        """Menyimpan konfigurasi aplikasi ke file JSON"""
        try:
            connected_serials = [
    ...
    def load_current_customer(self):
        """Load data customer dari file JSON"""
        if os.path.exists(self.customer_file):
            try:
                with open(self.customer_file, 'r', encoding='utf-8') as f:
                    self.current_customer = json.load(f)
                print(f"Customer data loaded: {self.current_customer.get('customer_name', 'None')}")
            except Exception as e:
                print(f"Failed to load customer data: {str(e)}")
                self.current_customer = {
                    'customer_code': '',
                    'customer_name': '',
                    'sales_code': '',
    ...
    def save_current_customer(self):
        """Menyimpan data customer ke file JSON"""
        try:
            with open(self.customer_file, 'w', encoding='utf-8') as f:
                json.dump(self.current_customer, f, indent=4, ensure_ascii=False)
            return True
        except Exception as e:
            print(f"Failed to save customer data: {str(e)}")
            return False

    def auto_reconnect_devices(self):
        """Auto-reconnect alat yang terakhir terhubung"""
...
# 2. ===SETTING GUI APLIKASI===
    ...
    def on_window_resize(self, event):
        """Menangani resize ukuran windows"""
    ...
    def manual_save_config(self):
        """Simpan konfigurasi secara manual"""
    ...
    def reset_configuration(self):
        """Menghapus semua konfigurasi yang tersimpan"""
...
# 3. ===SETTING TAB MENU TAMPILAN GUI===
    ...
    def create_socket_tab(self):
        ...
        socket_process_frame.grid(row=2, column=0, padx=10, pady=5, sticky='ew')
    ...
    def create_results_tab(self):
        ...
        self.results_frame.grid_rowconfigure(2, weight=0)  # Button frame (BARU)
        ...
        # Configure button columns for responsive layout
        for i in range(3):
            button_frame.grid_columnconfigure(i, weight=1)
        
        # Clear Results Button (prominent)
        ttk.Button(
            button_frame,
            text="Clear Results",
            command=self.clear_results_display,
            style="Accent.TButton"
        ).grid(row=0, column=0, padx=5, sticky='ew')
        
    def create_api_tab(self):
        """Tab untuk koneksi API"""
        # Configure grid weights
        self.api_frame.grid_rowconfigure(0, weight=0)  
        self.api_frame.grid_rowconfigure(1, weight=1)  
        self.api_frame.grid_rowconfigure(2, weight=0)
    ...
    def create_multi_serial_tab(self):
        ...
        main_paned.add(port_list_frame, weight=1)  # Initial weight
        ...
        main_paned.add(log_frame, weight=2)  # Initial weight (largest)
    ...
    def create_customer_tab(self):
        """Tab untuk Menu Customer"""
        self.customer_frame.grid_rowconfigure(0, weight=1)  # PanedWindow (expandable)
        self.customer_frame.grid_columnconfigure(0, weight=1)
        
        # ===== MAIN PANED WINDOW (VERTICAL - 3 SECTIONS) =====
        main_customer_paned = ttk.PanedWindow(self.customer_frame, orient=tk.VERTICAL)
        main_customer_paned.grid(row=0, column=0, padx=10, pady=10, sticky='nsew')
        
        # ===== PANE 1: CUSTOMER INFORMATION FORM =====
        form_frame = ttk.LabelFrame(main_customer_paned, text="Customer Information Form", padding=20)
        main_customer_paned.add(form_frame, weight=1)  # Initial weight
        
        form_frame.grid_columnconfigure(1, weight=1)
        
        # Customer Code
        ttk.Label(form_frame, text="Kode Customer:", font=("Arial", 10, "bold")).grid(
            row=0, column=0, sticky='w', pady=8, padx=5
        )
        self.customer_code_entry = ttk.Entry(form_frame, font=("Arial", 10))
        self.customer_code_entry.grid(row=0, column=1, pady=8, padx=5, sticky='ew')
        
        # Customer Name
        ttk.Label(form_frame, text="Nama Customer:", font=("Arial", 10, "bold")).grid(
            row=1, column=0, sticky='w', pady=8, padx=5
        )
        self.customer_name_entry = ttk.Entry(form_frame, font=("Arial", 10))
        self.customer_name_entry.grid(row=1, column=1, pady=8, padx=5, sticky='ew')
        
        # Sales Code
        ttk.Label(form_frame, text="Kode Sales:", font=("Arial", 10, "bold")).grid(
            row=2, column=0, sticky='w', pady=8, padx=5
        )
        self.sales_code_entry = ttk.Entry(form_frame, font=("Arial", 10))
        self.sales_code_entry.grid(row=2, column=1, pady=8, padx=5, sticky='ew')
        
        # Sales Name
        ttk.Label(form_frame, text="Nama Sales:", font=("Arial", 10, "bold")).grid(
            row=3, column=0, sticky='w', pady=8, padx=5
        )
        self.sales_name_entry = ttk.Entry(form_frame, font=("Arial", 10))
        self.sales_name_entry.grid(row=3, column=1, pady=8, padx=5, sticky='ew')
        ...
        ttk.Button(
            btn_frame, 
            text="💾 Save to Database", 
            command=self.save_customer_to_db,
            style="Accent.TButton"
        ).grid(row=0, column=0, padx=5, sticky='ew')
        ...
        ttk.Button(
            btn_frame, 
            text="✅ Set as Current", 
            command=self.set_current_customer,
            style="Accent.TButton"
        ...
        # ===== PANE 2: CUSTOMER DATABASE LIST =====
        list_frame = ttk.LabelFrame(main_customer_paned, text="Customer Database", padding=10)
        main_customer_paned.add(list_frame, weight=3)  # Larger weight (main content)
        
        list_frame.grid_rowconfigure(0, weight=1)
        list_frame.grid_columnconfigure(0, weight=1)

        columns = ('Customer Code', 'Customer Name', 'Sales Code', 'Sales Name')
        self.customer_tree = ttk.Treeview(list_frame, columns=columns, show='headings')

        self.customer_tree.heading('Customer Code', text='Kode Customer')
        self.customer_tree.heading('Customer Name', text='Nama Customer')
        self.customer_tree.heading('Sales Code', text='Kode Sales')
        self.customer_tree.heading('Sales Name', text='Nama Sales')

        self.customer_tree.column('Customer Code', width=150, minwidth=120)
        self.customer_tree.column('Customer Name', width=250, minwidth=200)
        self.customer_tree.column('Sales Code', width=120, minwidth=100)
        self.customer_tree.column('Sales Name', width=180, minwidth=150)
        
        v_scroll = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=self.customer_tree.yview)
        self.customer_tree.configure(yscrollcommand=v_scroll.set)
        h_scroll = ttk.Scrollbar(list_frame, orient=tk.HORIZONTAL, command=self.customer_tree.xview)
        self.customer_tree.configure(xscrollcommand=h_scroll.set)
        
        self.customer_tree.grid(row=0, column=0, sticky='nsew')
        v_scroll.grid(row=0, column=1, sticky='ns')
        h_scroll.grid(row=1, column=0, sticky='ew')
        
        self.customer_tree.bind('<Double-1>', self.load_customer_from_tree)
        ...
        ttk.Button(
            list_btn_frame,
            text="✅ Use as Current",
            command=self.use_selected_customer
        ).grid(row=0, column=1, padx=5, sticky='ew')
        ...
        # ===== PANE 3: CURRENT ACTIVE CUSTOMER =====
        current_frame = ttk.LabelFrame(main_customer_paned, text="Current Active Customer", padding=15)
        main_customer_paned.add(current_frame, weight=1)  # Smaller weight
        
        current_frame.grid_rowconfigure(0, weight=1)
        current_frame.grid_columnconfigure(0, weight=1)
        
        self.current_customer_label = tk.Label(
            current_frame,
            text=self.format_current_customer_display(),
            font=("Consolas", 10),
            bg='#e8f5e9',
            fg='#2e7d32',
            anchor='nw',
        ...
        self.current_customer_label.grid(row=0, column=0, sticky='nsew')
        
        self.refresh_customer_list()

    def format_current_customer_display(self):
        """Displat customer yang sedang aktif"""
        if not self.current_customer.get('customer_code'):
            return "No customer selected - Please select a customer to associate with test results"
        
        return (f"   Active Customer:\n"
                f"   Customer: [{self.current_customer['customer_code']}] {self.current_customer['customer_name']}\n"
                f"   Sales: [{self.current_customer['sales_code']}] {self.current_customer['sales_name']}")

    def save_customer_to_db(self):
        """Simpan data customer ke database"""
        customer_code = self.customer_code_entry.get().strip()
        customer_name = self.customer_name_entry.get().strip()
        sales_code = self.sales_code_entry.get().strip()
        sales_name = self.sales_name_entry.get().strip()
        
        # Validation
        if not customer_code:
            messagebox.showerror("Validation Error", "Kode Customer tidak boleh kosong!")
            self.customer_code_entry.focus_set()
            return
        
        if not customer_name:
            messagebox.showerror("Validation Error", "Nama Customer tidak boleh kosong!")
        ...
        def save_to_db():
            try:
                ...
                cur.execute("SELECT customer_id FROM customer WHERE customer_code = %s", (customer_code,))
                ...
                if existing:
                    ...
                    cur.execute("""
                        UPDATE customer 
                        SET customer_name = %s, sales_code = %s, sales_name = %s, updated_at = CURRENT_TIMESTAMP
                        WHERE customer_code = %s
                        RETURNING customer_id
                    """, (customer_name, sales_code, sales_name, customer_code))
                    ...
                    # Insert new
                    cur.execute("""
                        INSERT INTO customer (customer_code, customer_name, sales_code, sales_name)
                        VALUES (%s, %s, %s, %s)
                        RETURNING customer_id
                    """, (customer_code, customer_name, sales_code, sales_name))
                ...
                self.root.after(0, lambda: messagebox.showinfo(
                    "Success", 
                    f"Customer {action} successfully!\n\n"
                    f"ID: {customer_id}\n"
                    f"Code: {customer_code}\n"
                    f"Name: {customer_name}"
                ))
                self.root.after(0, self.refresh_customer_list)
                self.root.after(0, lambda: self.log_multi_serial(
                    f"Customer {action}: [{customer_code}] {customer_name}"
                ))
    ...
    def set_current_customer(self):
        """Mengatur form data sebagai customer yang aktif"""
        customer_code = self.customer_code_entry.get().strip()
        customer_name = self.customer_name_entry.get().strip()
        sales_code = self.sales_code_entry.get().strip()
        sales_name = self.sales_name_entry.get().strip()
        ...
        if not customer_code or not customer_name:
            messagebox.showwarning("Warning", "Please fill at least Customer Code and Name!")
            return
        ...
        self.save_current_customer()
        self.current_customer_label.configure(text=self.format_current_customer_display())
        
        messagebox.showinfo(
            "Success",
            f"Current customer set to:\n\n"
            f"[{customer_code}] {customer_name}\n"
            f"Sales: [{sales_code}] {sales_name}"
        )
        self.log_multi_serial(f"Current customer: [{customer_code}] {customer_name}")

    def refresh_customer_list(self):
        """Refresh data customer dari database"""
        def fetch_customers():
            try:
                self.update_config()
                conn = psycopg2.connect(**self.db_config)
                ...
                cur.execute("""
                    SELECT customer_id, customer_code, customer_name, sales_code, sales_name, 
                        created_at 
                    FROM customer 
                ...
                self.root.after(0, lambda: self.populate_customer_tree(customers))
        ...
        threading.Thread(target=fetch_customers, daemon=True).start()

    def populate_customer_tree(self, customers):
        for item in self.customer_tree.get_children():
            self.customer_tree.delete(item)
        
        for customer in customers:
            customer_id, code, name, sales_code, sales_name, created = customer
            
            item_id = self.customer_tree.insert('', tk.END, 
                values=(
                    code,                                      
                    name,                                      
                    sales_code if sales_code else '-',        
                    sales_name if sales_name else '-'         
                ),
                tags=(str(customer_id),)  
            )

    def load_customer_from_tree(self, event):
        self.edit_selected_customer()

    def edit_selected_customer(self):
        """Edit customer dari list yang dipilih"""
        ...
        self.sales_code_entry.delete(0, tk.END)
        self.sales_code_entry.insert(0, values[2] if values[2] != '-' else '')
        
        self.sales_name_entry.delete(0, tk.END)
        self.sales_name_entry.insert(0, values[3] if values[3] != '-' else '')

        self.customer_name_entry.focus_set()
        self.customer_name_entry.select_range(0, tk.END)
        
        if hasattr(self, 'customer_status_label'):
            self.customer_status_label.configure(
                text=f"Editing: [{values[0]}] {values[1]}", 
                fg='#3498db'
            )
        
        self.log_multi_serial(f"Editing customer: [{values[0]}] {values[1]}")         
        messagebox.showinfo(
            "Edit Mode",
            f"Customer data loaded to form:\n\n"
            f"Code: {values[0]}\n"   
            f"Name: {values[1]}\n\n" 
            f"Please modify the data and click 'Save to Database' to update."
        )

    def use_selected_customer(self):
        """Menggunakan customer yang dipilih dari list sebagai customer aktif"""
        selected = self.customer_tree.selection()
        if not selected:
            messagebox.showwarning("Warning", "Please select a customer first!")
            return
        ...
        self.current_customer = {
            ...
            'sales_code': values[2] if values[2] != '-' else '',
            'sales_name': values[3] if values[3] != '-' else ''
        }
        ...
        messagebox.showinfo(
            ...
            f"Current customer set to:\n\n"
            f"[{values[0]}] {values[1]}\n"      # FIX: index 0 dan 1
            f"Sales: [{values[2]}] {values[3]}" # FIX: index 2 dan 3
        )
        self.log_multi_serial(f"Current customer: [{values[0]}] {values[1]}")  # FIX

    def delete_selected_customer(self):
        """Soft delete untuk menghapus customer yang dipilih"""
        selected = self.customer_tree.selection()
        if not selected:
            messagebox.showwarning("Warning", "Please select a customer first!")
            return
        ...
        customer_id = int(tags[0]) if tags else None
        customer_code = values[0]
        customer_name = values[1]
        
        if not customer_id:
            messagebox.showerror("Error", "Cannot identify customer ID")
            return
        ...
        if not messagebox.askyesno(
            ...
            f"Code: {customer_code}\n"
            f"Name: {customer_name}\n\n"
            f"This will mark the customer as inactive."
        ):
            return
        ...
        def delete_from_db():
            try:
                ...
                self.root.after(0, self.refresh_customer_list)
                self.root.after(0, lambda: self.log_multi_serial(
                    f"Customer deleted: [{customer_code}] {customer_name}"
                ))
...
# 4. ===SETTING MENU INPUT FILE===
    ...
    def clear_file_content(self):
        """Membersihkan konten file dan hasil"""
        self.hl7_text.configure(state=tk.NORMAL)
        self.hl7_text.delete(1.0, tk.END)
        self.hl7_text.configure(state=tk.DISABLED)
...
# 5. ===SETTING LOGIKA PARSE===
    def strip_mllp_wrapper(self, raw_data):
        """Menghapus MLLP wrapper dari format HL7 menghasilkan HL7 yang bersih"""
        try:
            # Karakter kontrol MLLP (hexadecimal)
            VT = '\x0B'   # Start Block (0x0B)
            FS = '\x1C'   # End Block (0x1C) 
            CR = '\x0D'   # Carriage Return (0x0D)
            
            clean_data = raw_data
            
            # STEP 1: Remove Start Block (VT) di awal
            if clean_data.startswith(VT):
                clean_data = clean_data[1:]
                self.log_api_response("MLLP: Start Block (<VT>) removed")
            
            # STEP 2: Remove End Block (FS) dan Carriage Return (CR) di akhir
            # Format bisa <FS><CR> atau hanya <FS>
            if clean_data.endswith(FS + CR):
                clean_data = clean_data[:-2]
                self.log_api_response("MLLP: End markers (<FS><CR>) removed")
            elif clean_data.endswith(FS):
                clean_data = clean_data[:-1]
                self.log_api_response("MLLP: End marker (<FS>) removed")
            
            # STEP 3: Replace internal <CR> dengan newline untuk parsing
            clean_data = clean_data.replace(CR, '\n')
            
            self.log_api_response(
                f"MLLP wrapper stripped: {len(raw_data)} bytes → {len(clean_data)} bytes"
            )
            
            return clean_data.strip()
            
        except Exception as e:
            self.log_api_response(f"Error stripping MLLP: {str(e)}")
            return raw_data
    ...
    def parse_hl7(self, hl7_text):
        ...
        # Device type flags
        is_bc5300_device = False
        is_urit_device = False
        is_hematology_device = False
        
        # Hematology device keywords (specific models only, not brands!)
        hematology_keywords = ['BC-5300', 'BC5300', 'BC-1800', 'BC1800', 
                            'BC-3200', 'BC3200', 'BC-2600', 'BC2600',
                            'BC-']
        ...
        for line_num, line in enumerate(lines):
            ...
            if segment_type == 'MSH':
                try:
                    # Parse MSH fields for device detection
                    msh_parts = segments  # Already split by |
                    
                    # Field 2: Sending Application (index 2)
                    # Field 3: Sending Facility (index 3)
                    sending_app = msh_parts[2].upper() if len(msh_parts) > 2 else ''
                    sending_facility = msh_parts[3].upper() if len(msh_parts) > 3 else ''
                    
                    # DEVICE DETECTION PRIORITY
                    
                    # Priority 1: BC-5300 (exact model match in field 2 or 3)
                    if 'BC-5300' in sending_app or 'BC5300' in sending_app or \
                    'BC-5300' in sending_facility or 'BC5300' in sending_facility:
                        is_bc5300_device = True
                        is_hematology_device = True
                        self.log_api_response(f"BC-5300 detected | App: '{sending_app}', Facility: '{sending_facility}'")
                    
                    # Priority 2: URIT (brand or model number)
                    elif 'URIT' in sending_app or 'URIT' in sending_facility or \
                        '8030' in sending_facility or '8031' in sending_facility:
                        is_urit_device = True
                        self.log_api_response(f"URIT detected | App: '{sending_app}', Facility: '{sending_facility}'")
                    
                    # Priority 3: Other hematology devices
                    else:
                        check_string = f"{sending_app}|{sending_facility}"
                        for keyword in hematology_keywords:
                            if keyword in check_string:
                                is_hematology_device = True
                                self.log_api_response(f"Hematology device detected: {keyword}")
                                break
                        
                        # Log standard device
                        if not is_hematology_device:
                            self.log_api_response(f"Standard HL7 device | App: '{sending_app}', Facility: '{sending_facility}'")
                    ...
                    if len(segments) > 6 and segments[6]:
                        ...
                        if len(msg_datetime) >= 14:
                            ...
                            self.log_api_response(f"Sample time from MSH: {obr_sample_time}")
                        elif len(msg_datetime) >= 8:
                            year = msg_datetime[:4]
                            month = msg_datetime[4:6]
                            day = msg_datetime[6:8]
                            obr_sample_time = f"{year}-{month}-{day}"
                            self.log_api_response(f"Sample date from MSH: {obr_sample_time}")
                            
                except Exception as e:
                    self.log_api_response(f"Error parsing MSH: {str(e)}")

            # PID SEGMENT (Patient Identification)
            elif segment_type == 'PID':
                try:
                    # SKIP PID for BC-5300 and URIT (they use OBR for Patient ID)
                    if is_bc5300_device or is_urit_device:
                        device_name = "BC-5300" if is_bc5300_device else "URIT"
                        self.log_api_response(f"Skipping PID for {device_name} - Patient ID will be extracted from OBR")
                    
                    # STANDARD DEVICES: Try to extract Patient ID from PID fields
                    else:
                        # Try Field 2 (index 1)
                        if not obr_patient_id and len(segments) > 1 and segments[1]:
                            ...
                            if self.is_valid_patient_id(field_value):
                                ...
                                self.log_api_response(f"Patient ID from PID field 2: '{obr_patient_id}'")
                        ...
                        if not obr_patient_id and len(segments) > 2 and segments[2]:
                            ...
                            if self.is_valid_patient_id(field_value):
                                obr_patient_id = field_value
                                self.log_api_response(f"Patient ID from PID field 3: '{obr_patient_id}'")
                        ...
                        if not obr_patient_id and len(segments) > 3 and segments[3]:
                            ...
                            if self.is_valid_patient_id(field_value):
                                obr_patient_id = field_value
                                self.log_api_response(f"Patient ID from PID field 4: '{obr_patient_id}'")
                    
                    # Field 5: Patient Name (all devices)
                    if len(segments) > 4 and segments[4]:
                        ...
                        if pid_patient_name and pid_patient_name not in ['^', '', 'NOERS', '0']:
                            ...
                            self.log_api_response(f"Patient name: '{pid_patient_name}'")
                            
                except Exception as e:
                    self.log_api_response(f"Error parsing PID: {str(e)}")

            # OBR SEGMENT (Observation Request)
            elif segment_type == 'OBR':
                try:
                    # ===== BC-5300: Patient ID from field 4 sub-component =====
                    if is_bc5300_device:
                        if len(segments) > 4 and segments[4]:
                            # Format: 00001^Automated Count^99MRC
                            field_4_parts = segments[4].split('^')
                            if field_4_parts and field_4_parts[0].strip():
                                patient_id_candidate = field_4_parts[0].strip()
                                # Validate: not empty, not invalid characters
                                if patient_id_candidate and patient_id_candidate not in ['^', '', 'N', 'n']:
                                    obr_patient_id = patient_id_candidate
                                    self.log_api_response(f"BC-5300 Patient ID from OBR field 4: '{obr_patient_id}'")
                    
                    # ===== URIT: Patient ID from field 3 =====
                    elif is_urit_device:
                        if len(segments) > 3 and segments[3]:
                            order_number = segments[3].strip()
                            if order_number and order_number not in ['^', '', 'N', 'n']:
                                obr_patient_id = order_number
                                self.log_api_response(f"URIT Patient ID from OBR field 3: '{obr_patient_id}'")
                    
                    # ===== STANDARD DEVICES: Patient ID from field 2 or field 3 =====
                    else:
                        # Priority 1: Field 2 (Placer Order Number)
                        if not obr_patient_id and len(segments) > 2 and segments[2]:
                            placer_order = segments[2].strip()
                            # Don't use is_valid_patient_id - order numbers are valid as Patient IDs
                            if placer_order and placer_order not in ['^', '', 'N', 'n']:
                                obr_patient_id = placer_order
                                self.log_api_response(f"Patient ID from OBR field 2 (Placer Order): '{obr_patient_id}'")
                        
                        # Priority 2: Field 3 (Filler Order Number) - fallback
                        if not obr_patient_id and len(segments) > 3 and segments[3]:
                            filler_order = segments[3].strip()
                            if filler_order and filler_order not in ['^', '', 'N', 'n']:
                                obr_patient_id = filler_order
                                self.log_api_response(f"Patient ID from OBR field 3 (Filler Order): '{obr_patient_id}'")
                    
                    # Field 7: Observation Date/Time (fallback if MSH empty)
                    if not obr_sample_time and len(segments) > 7 and segments[7]:
                        sample_time_raw = segments[7].strip()
                        
                        # Try YYYYMMDDHHMMSS or YYYY-MM-DD format
                        if len(sample_time_raw) >= 14 and sample_time_raw.replace('-', '').replace(':', '').isdigit():
                            if '-' in sample_time_raw:
                                ...
                                obr_sample_time = f"{year}-{month}-{day} {hour}:{minute}:{second}"
                            self.log_api_response(f"Sample time from OBR: {obr_sample_time}")
                            ...
                            if '-' in sample_time_raw:
                                ...
                                day = sample_time_raw[6:8]
                                obr_sample_time = f"{year}-{month}-{day}"
                            self.log_api_response(f"Sample date from OBR: {obr_sample_time}")
                            
                except Exception as e:
                    self.log_api_response(f"Error parsing OBR: {str(e)}")

            # OBX SEGMENT (Observation Results)
            elif segment_type == 'OBX':
                # SKIP OBX parsing for hematology devices (including BC-5300)
                if is_hematology_device:
                    continue
                
                # PARSE OBX for URIT and Standard devices - VALUES SET TO '-'
                try:
                    if len(segments) > 5:
                        # Field 3: Observation Identifier
                        obs_id = segments[3].split('^') if len(segments) > 3 else []
                        
                        # Determine test name based on format
                        if len(obs_id) > 1:
                            # Standard HL7 with ^ separator (e.g., BS-200)
                            test_name = obs_id[1] if obs_id[1] else obs_id[0]
                        elif obs_id and obs_id[0].isdigit():
                            # URIT format (test number in field 3)
                            test_name = segments[4] if len(segments) > 4 else 'Unknown Test'
                        else:
                            # Other formats
                            test_name = obs_id[0] if obs_id else 'Unknown Test'

                        # MODIFIED: Set all values to '-'
                        value = '-'
                        units = '-'
                        ref_range = '-'
                        abnormal_flag = '-'

                        # Field 14: Observation Date/Time (optional)
                        obs_time = ''
                        ...
                        # Add result if test name exists
                        if test_name:
                            results.append({
                                'test_name': test_name,
                                'value': value,
                    ...
                    self.log_api_response(f"Error parsing OBX: {str(e)}")

        # Use patient name as fallback if no Patient ID found
        ...
        if is_bc5300_device:
            device_type = "BC-5300 (Hematology)"
        elif is_urit_device:
            device_type = "URIT"
        elif is_hematology_device:
            device_type = "Hematology Device"
        else:
            device_type = "Standard HL7"
        
        # Final log
        self.log_api_response(
            f"PARSING COMPLETE | Device: {device_type} | "
            f"Patient ID: '{patient['patient_id']}' | "
            f"Sample Time: '{patient['sample_time']}' | "
    ...
    def detect_data_format(self, data):
        ...
        # PRIORITY 0: MLLP Wrapped HL7 (detect by control characters)
        if data_stripped.startswith('\x0B') and ('\x1C' in data_stripped):
            return "MLLP_HL7"
    ...
    def parse_bc1800(self, raw_data):
        ...
        try:
            ...
            try:
                ...
                mmdd = clean_data[21:25] 
                yyyy = clean_data[25:29] 
                hhmm = clean_data[29:33] 
    ...
    def parse_data_auto(self, data):
        ...
        try:
            ...
            # ===== TAMBAHKAN INI SETELAH DETEKSI =====
            if data_format == "MLLP_HL7":
                self.log_api_response("Detected: MLLP-wrapped HL7 format")
                # Strip MLLP wrapper dulu
                clean_hl7 = self.strip_mllp_wrapper(data)
                # Parse sebagai HL7 biasa
                return self.parse_hl7(clean_hl7)

            elif data_format == "BC1800":
    ...
    def is_complete_message(self, data):
        """Universal message completeness detection"""
        data = data.strip()
        data_upper = data.upper()

        # CHECK 0: MLLP Wrapped HL7
        if data.startswith('\x0B'):  # Start Block detected
            # Complete jika ada End Block + CR
            if data.endswith('\x1C\x0D') or data.endswith('\x1C'):
                return True, "MLLP_HL7"
            # Jika sudah cukup besar tapi belum ada end marker (timeout fallback)
            if len(data) > 500:
                return True, "MLLP_HL7"
            return False, "MLLP_HL7"
    ...
    def save_to_database_with_context(self, patient, results, data_format, 
        ...
        def save_data():
            try:
                ...
                # FIX: Get device info from device_labels and extract strings
                device_info = self.device_labels[device_type].get(device_identifier, {})
                if isinstance(device_info, dict):
                    # Extract label as string
                    actual_device_label = device_info.get("label", "")
                    serial_number = device_info.get("serial_number", "")
                    device_category = device_info.get("device_type", "")
                else:
                    # Old format compatibility (device_info is already a string)
                    actual_device_label = device_info if device_info else ""
                    serial_number = ""
                    device_category = ""
                
                # Use default label if empty
                if not actual_device_label:
                    actual_device_label = f"Unlabeled {device_type.capitalize()} ({device_identifier})"
                
                # Get or create device with enhanced information
                cur.execute("""
                    SELECT get_or_create_device(
                        %s::VARCHAR,      -- device_label
                        %s::VARCHAR,      -- device_type (socket/serial)
                        %s::VARCHAR,      -- device_identifier (IP/Port)
                        %s::VARCHAR,      -- serial_number
                        %s::VARCHAR       -- device_category (Hematology/Chemistry/etc)
                    )
                """, (actual_device_label, device_type, device_identifier, serial_number, device_category))
                ...
                if data_format in ["HL7", "CUSTOM_HL7", "URIT_8030"] and len(results) > 0:
                    for r in results:
                        # MODIFIED: Ensure '-' is saved for all value fields
                        test_name = r.get('test_name', '')
                        test_value = r.get('value', '-')
                        test_units = r.get('units', '-')
                        test_ref_range = r.get('reference_range', '-')
                        abnormal_flag = r.get('abnormal_flag', '-')
                        
                        # Convert empty strings to '-'
                        test_value = test_value if test_value and test_value.strip() else '-'
                        test_units = test_units if test_units and test_units.strip() else '-'
                        test_ref_range = test_ref_range if test_ref_range and test_ref_range.strip() else '-'
                        abnormal_flag = abnormal_flag if abnormal_flag and abnormal_flag.strip() else '-'
                ...
                # Success message - FIX: Use actual_device_label (string) not device_label (could be dict)
                success_msg = f"[{device_identifier}] Saved to DB | Record #{record_id} | Device: {actual_device_label}"
                if serial_number:
                    success_msg += f" | S/N: {serial_number}"
                if device_category:
                    success_msg += f" | Type: {device_category}"
...
# 7. ===SETTING SOCKET SERVER METHODS===
    ...
    def display_received_data(self, data):
        ...
        self.log_socket_message(f"Data received ({len(data)} bytes) - processing...")
    ...
    def process_and_save_with_context(self, raw_data, device_type, device_identifier):
        ...
        """
        try:
            # FIX: Get device label as STRING (not dict)
            device_info = self.device_labels[device_type].get(device_identifier, {})
            
            if isinstance(device_info, dict):
                device_label = device_info.get("label", "")
                if not device_label:
                    device_label = f"Unlabeled {device_type.capitalize()} ({device_identifier})"
            else:
                # Old format compatibility (already a string)
                device_label = device_info if device_info else f"Unlabeled {device_type.capitalize()} ({device_identifier})"
            ...
            # FIX: Pass device_label as STRING (not dict)
            self.save_to_database_with_context(
                patient=patient,
                results=results,
                data_format=data_format,
                device_type=device_type,
                device_identifier=device_identifier,
                device_label=device_label  # This is now a STRING
            )
...
# 8. ===SETTING RESULTS TAB MENU
    ...
    def update_results_display_with_context(self, patient, results, data_format, device_source):
        ...
        try:
            # AUTO-CLEAR: Clear patient info (data lama otomatis terhapus)
            self.patient_info.delete(1.0, tk.END)
            ...
            # AUTO-CLEAR: Clear and populate results tree (data lama otomatis terhapus)
            for item in self.results_tree.get_children():
                self.results_tree.delete(item)
            ...
            if data_format in ["HL7", "Custom HL7", "URIT-8030"]:
                for result in results:
                    # MODIFIED: Ensure all values are '-'
                    ...
                    # Convert empty strings to '-'
                    value = value if value and value.strip() else '-'
                    units = units if units and units.strip() else '-'
                    ref_range = ref_range if ref_range and ref_range.strip() else '-'
                    flag = flag if flag and flag.strip() else '-'
    ...
    def clear_results_display(self):
        """Clear all results display in Results tab"""
        if messagebox.askyesno("Confirm Clear", 
            ...
            # Update status
            self.update_status("Results cleared - Ready for new data")
            
            # Log to all relevant logs
            self.log_multi_serial("Results display cleared by user")
            self.log_socket_message("Results display cleared by user")
            self.log_api_response("Results display cleared by user")
    ...
    def show_about(self):
        """Show about dialog"""
        about_text = """Data Parser

Features:
• All tools using MLLP HL7 format
...
# 9. ===SETTING KONEKSI DAN LOGIKA PORT===
    ...
    def clear_all_serial_data(self):
        """Clear all serial data - DEPRECATED, use clear_results_display() instead"""
    ...
    def display_serial_received_data(self, data, port_name):
        ...
        self.log_multi_serial(f"[{port_name}] Data received ({len(data)} bytes) - processing...")
    
# 10. ===SETTING TAB MENU LABEL DEVICE===
    def create_socket_label_tab(self, parent):
        """Create Socket Device Labels tab with enhanced fields"""
        ...
        # Treeview with additional columns
        columns = ("IP Address", "Device Label", "Serial Number", "Device Type")
        self.socket_label_tree = ttk.Treeview(frame, columns=columns, show="headings")
        
        # Configure columns
        self.socket_label_tree.heading("IP Address", text="IP Address")
        self.socket_label_tree.heading("Device Label", text="Device Label")
        self.socket_label_tree.heading("Serial Number", text="Serial Number")
        self.socket_label_tree.heading("Device Type", text="Device Type")
        
        self.socket_label_tree.column("IP Address", width=150, minwidth=100)
        self.socket_label_tree.column("Device Label", width=200, minwidth=150)
        self.socket_label_tree.column("Serial Number", width=150, minwidth=100)
        self.socket_label_tree.column("Device Type", width=150, minwidth=100)
        
        self.socket_label_tree.grid(row=0, column=0, sticky="nsew")

        # Scrollbars
        v_scrollbar = ttk.Scrollbar(frame, orient="vertical", command=self.socket_label_tree.yview)
        self.socket_label_tree.configure(yscrollcommand=v_scrollbar.set)
        v_scrollbar.grid(row=0, column=1, sticky="ns")
        
        h_scrollbar = ttk.Scrollbar(frame, orient="horizontal", command=self.socket_label_tree.xview)
        self.socket_label_tree.configure(xscrollcommand=h_scrollbar.set)
        h_scrollbar.grid(row=1, column=0, sticky="ew")
        ...
        ttk.Button(btn_frame, text="Add / Update Device", 
                command=self.add_socket_label, 
                style="Accent.TButton").grid(row=0, column=0, padx=5, sticky="ew")
        ttk.Button(btn_frame, text="Delete Selected", 
                command=self.delete_socket_label).grid(row=0, column=1, padx=5, sticky="ew")
        ttk.Button(btn_frame, text="Save Labels", 
                command=self.save_device_labels,
                style="Accent.TButton").grid(row=0, column=2, padx=5, sticky="ew")

        self.refresh_socket_label_tree()

    def create_serial_label_tab(self, parent):
        """Create Serial Device Labels tab with enhanced fields"""
        parent.grid_rowconfigure(0, weight=1)
        parent.grid_columnconfigure(0, weight=1)
        ...
        # Treeview with additional columns
        columns = ("Port", "Device Label", "Serial Number", "Device Type")
        self.serial_label_tree = ttk.Treeview(frame, columns=columns, show="headings")
        
        # Configure columns
        self.serial_label_tree.heading("Port", text="Port")
        self.serial_label_tree.heading("Device Label", text="Device Label")
        self.serial_label_tree.heading("Serial Number", text="Serial Number")
        self.serial_label_tree.heading("Device Type", text="Device Type")
        
        self.serial_label_tree.column("Port", width=150, minwidth=100)
        self.serial_label_tree.column("Device Label", width=200, minwidth=150)
        self.serial_label_tree.column("Serial Number", width=150, minwidth=100)
        self.serial_label_tree.column("Device Type", width=150, minwidth=100)
        
        self.serial_label_tree.grid(row=0, column=0, sticky="nsew")

        # Scrollbars
        v_scrollbar = ttk.Scrollbar(frame, orient="vertical", command=self.serial_label_tree.yview)
        self.serial_label_tree.configure(yscrollcommand=v_scrollbar.set)
        v_scrollbar.grid(row=0, column=1, sticky="ns")
        
        h_scrollbar = ttk.Scrollbar(frame, orient="horizontal", command=self.serial_label_tree.xview)
        self.serial_label_tree.configure(xscrollcommand=h_scrollbar.set)
        h_scrollbar.grid(row=1, column=0, sticky="ew")
        ...
        ttk.Button(btn_frame, text="Add / Update Device", 
                command=self.add_serial_label,
                style="Accent.TButton").grid(row=0, column=0, padx=5, sticky="ew")
        ttk.Button(btn_frame, text="Delete Selected", 
                command=self.delete_serial_label).grid(row=0, column=1, padx=5, sticky="ew")
        ttk.Button(btn_frame, text="Save Labels", 
                command=self.save_device_labels,
                style="Accent.TButton").grid(row=0, column=2, padx=5, sticky="ew")
    ...
    def add_socket_label(self):
        """Add or update socket device information with dialog"""
        selected = self.socket_label_tree.selection()
        
        # Get IP and current values
        if selected:
            item = self.socket_label_tree.item(selected[0])
            ip = item["values"][0]
            current_data = self.device_labels["socket"].get(ip, {
                "label": "",
                "serial_number": "",
                "device_type": ""
            })
        ...
        # Create dialog
        dialog = tk.Toplevel(self.root)
        dialog.title(f"Device Information - {ip}")
        ...
        # Main frame
        main_frame = ttk.LabelFrame(dialog, text=f"Socket Device: {ip}", padding=20)
        main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        main_frame.grid_columnconfigure(1, weight=1)
        
        # Device Label
        ttk.Label(main_frame, text="Device Label:", font=("Arial", 10, "bold")).grid(
            row=0, column=0, sticky='w', pady=10, padx=5
        )
        label_var = tk.StringVar(value=current_data.get("label", ""))
        label_entry = ttk.Entry(main_frame, textvariable=label_var, font=("Arial", 10))
        label_entry.grid(row=0, column=1, pady=10, padx=5, sticky='ew')
        
        # Serial Number
        ttk.Label(main_frame, text="Serial Number:", font=("Arial", 10, "bold")).grid(
            row=1, column=0, sticky='w', pady=10, padx=5
        )
        serial_var = tk.StringVar(value=current_data.get("serial_number", ""))
        serial_entry = ttk.Entry(main_frame, textvariable=serial_var, font=("Arial", 10))
        serial_entry.grid(row=1, column=1, pady=10, padx=5, sticky='ew')
        
        # Device Type
        ttk.Label(main_frame, text="Device Type:", font=("Arial", 10, "bold")).grid(
            row=2, column=0, sticky='w', pady=10, padx=5
        )
        device_type_var = tk.StringVar(value=current_data.get("device_type", ""))
        ...
        device_type_combo.grid(row=2, column=1, pady=10, padx=5, sticky='ew')
        
        # Info label
        info_label = ttk.Label(
            main_frame,
            text="Note: This information will be saved to database\nwhen device sends data.",
            font=("Arial", 9),
            foreground="#7f8c8d",
            justify=tk.LEFT
        )
        info_label.grid(row=3, column=0, columnspan=2, pady=15, sticky='w')
        ...
        def save_and_close():
            ...
            if not label:
                messagebox.showwarning("Warning", "Device Label is required!")
                return
            
            # Save to device_labels
            self.device_labels["socket"][ip] = {
                "label": label,
            ...
            self.log_socket_message(
                f"Device info updated for {ip}: Label='{label}', "
                f"Serial='{serial}', Type='{dev_type}'"
            )
            messagebox.showinfo("Success", f"Device information saved for {ip}")
            dialog.destroy()
    ...
    def refresh_socket_label_tree(self):
        """Refresh socket label treeview with enhanced data"""
        for i in self.socket_label_tree.get_children():
            self.socket_label_tree.delete(i)
        
        for ip, data in self.device_labels["socket"].items():
            ...
            self.socket_label_tree.insert("", tk.END, values=(ip, label, serial, dev_type))

    def add_serial_label(self):
        """Add or update serial device information with dialog"""
        selected = self.serial_label_tree.selection()
        
        # Get Port and current values
        if selected:
            item = self.serial_label_tree.item(selected[0])
            port = item["values"][0]
            current_data = self.device_labels["serial"].get(port, {
                "label": "",
                "serial_number": "",
                "device_type": ""
            })
        ...
        # Create dialog
        dialog = tk.Toplevel(self.root)
        dialog.title(f"Device Information - {port}")
        dialog.geometry("500x350")
        dialog.transient(self.root)
        dialog.grab_set()
        ...
        # Main frame
        main_frame = ttk.LabelFrame(dialog, text=f"Serial Device: {port}", padding=20)
        main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        main_frame.grid_columnconfigure(1, weight=1)
        ...
        def save_and_close():
            ...
            # Save to device_labels
            self.device_labels["serial"][port] = {
                "label": label,
                "serial_number": serial,
                "device_type": dev_type
            }
            ...
            self.log_multi_serial(
                f"Device info updated for {port}: Label='{label}', "
                f"Serial='{serial}', Type='{dev_type}'"
            )
            messagebox.showinfo("Success", f"Device information saved for {port}")
            dialog.destroy()
    ...
    def refresh_serial_label_tree(self):
        """Refresh serial label treeview with enhanced data"""
        for i in self.serial_label_tree.get_children():
            self.serial_label_tree.delete(i)
        
        for port, data in self.device_labels["serial"].items():
            if isinstance(data, dict):
                label = data.get("label", "")
                serial = data.get("serial_number", "")
                dev_type = data.get("device_type", "")
            ...
            self.serial_label_tree.insert("", tk.END, values=(port, label, serial, dev_type))
    ...
    def get_device_label(self, device_type, device_identifier):
        """Get device label, handling both old string format and new dict format"""
        device_info = self.device_labels[device_type].get(device_identifier)
        
        if isinstance(device_info, dict):
            label = device_info.get("label", "")
            if not label:
                return f"Unlabeled {device_type.capitalize()} ({device_identifier})"
            return label
        elif isinstance(device_info, str):
            # Old format compatibility
            return device_info if device_info else f"Unlabeled {device_type.capitalize()} ({device_identifier})"
        else:
            return f"Unlabeled {device_type.capitalize()} ({device_identifier})"
        ...
        try:
            ...
            if data_format in ["HL7", "CUSTOM_HL7", "URIT_8030"] and hasattr(self, 'results') and len(self.results) > 0:
                ...
                for result in self.results:
                    ...
                    # Determine status (all '-' should be normal)
                    if abnormal_flag == '-':
                        status = 'normal'
                    elif abnormal_flag.upper() in ['NORMAL', 'N', 'normal', 'n', '']:
                        status = 'normal'
                    else:
                        status = 'abnormal'
            ...
            elif data_format in ["CUSTOM_HL7", "URIT_8030"]:
