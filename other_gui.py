...
# 1. ===SETTING INISIALISASI===
    ...
    def load_app_configuration(self):
        ...
        if os.path.exists(self.config_file):
            try:
                ...
                print(f"⚠️ Failed to load configuration: {str(e)}")  # ← GUNAKAN print()
                return False
        else:
            print("ℹ️  No saved configuration found - using defaults")  # ← GUNAKAN print()
            return False
    ...
    def auto_reconnect_devices(self):
        ...
        if not self.auto_startup_enabled:
            self.log_multi_serial("ℹ️  Auto-startup is disabled")
            return
        ...
        if self.db_config['host'] and self.db_config['database']:
            self.log_multi_serial("🗄️  Testing database connection...")
            try:
                ...
                def test_db():
                    try:
                        ...
                        self.root.after(0, lambda: self.log_multi_serial(f"⚠️ Database connection failed: {str(e)}"))
                
                threading.Thread(target=test_db, daemon=True).start()
                
            except Exception as e:
                self.log_multi_serial(f"⚠️ Database test error: {str(e)}")
            ...
            for port_name in self.last_connected_serials:
                if port_name in self.serial_configs:
                    try:
                        ...
                        self.log_multi_serial(f"   ⚠️ Failed to reconnect {port_name}: {str(e)}")
                else:
                    self.log_multi_serial(f"   ⚠️ Configuration for {port_name} not found")
        ...
        if self.socket_was_running:
            try:
                ...
                self.log_multi_serial(f"⚠️ Failed to start socket server: {str(e)}")
...
# 2. ===SETTING GUI APLIKASI===
    ...
    def toggle_auto_startup(self):
        ...
        if self.auto_startup_enabled:
            ...
            self.log_multi_serial("⏸️ Auto-startup DISABLED")
...
# 5. ===SETTING LOGIKA PARSE===
        ...
        for line in lines:
            ...
            if segment_type == 'MSH':
                try:
                    ...
                    self.log_api_response(f"⚠️ Warning: Error parsing MSH segment: {str(e)}")
            ...
            for line in lines:
                ...
                # ===== PID Segment =====
                    ...
                    try:
                        ...
                        self.log_api_response(f"⚠️ Warning: Error parsing PID segment: {str(e)}")
            ...
            for line in lines:
                ...
                # ===== OBX Segment (Observation Results) =====
                    ...
                    try:
                        ...
                        self.log_api_response(f"⚠️ Warning: Error parsing OBX segment: {str(e)}")
                else:
    ...
    def parse_data(self):
        ...
        try:
            ...
            # ✅ NEW: Tentukan device source untuk display
            device_source = "File Import (Manual)"
            
            # Display patient info dengan device source
            ...
            # ✅ FIX: Total Results for HL7, Custom HL7, and URIT-8030 ONLY
            if data_format in ["HL7", "CUSTOM_HL7", "URIT_8030"]:
                for result in self.results:
                    flag = result.get('abnormal_flag', '').strip()
            
                    if flag.upper() in ['NORMAL', 'N', 'normal', 'n', '']:
                        flag = 'Normal'
                    else:
                        flag = flag
    ...
    def is_complete_message(self, data):
        ...
        if "MSH" in data:
            if "OBX" in data or len(data) > 200:
                return True, "HL7"
...
# 6. ===SETTING MENU KONEKSI DATABASE===
    ...
    def save_to_database(self):
        """Save parsed data to database - NEW SCHEMA with proper device labeling"""
        ...
        def save_data():
            try:
                ...
                # ===== 1. GET DEVICE INFO (FIXED) =====
                device_label = None
                device_type = None
                device_identifier = None
                
                # FIX: Determine device type & identifier with proper priority
                # Priority 1: Check if data came from socket
                if hasattr(self, 'socket_running') and self.socket_running and hasattr(self, 'current_socket_ip'):
                    device_type = 'socket'
                    device_identifier = self.current_socket_ip
                    # Get label from socket devices
                    device_label = self.device_labels["socket"].get(device_identifier, f"Unlabeled Socket ({device_identifier})")
                    self.log_multi_serial(f"Device source: Socket | IP: {device_identifier} | Label: {device_label}")
                
                # Priority 2: Check if data came from serial port
                elif hasattr(self, 'current_serial_port') and self.current_serial_port:
                    device_type = 'serial'
                    device_identifier = self.current_serial_port
                    # Get label from serial devices
                    device_label = self.device_labels["serial"].get(device_identifier, f"Unlabeled Serial ({device_identifier})")
                    self.log_multi_serial(f"Device source: Serial | Port: {device_identifier} | Label: {device_label}")
                
                # Priority 3: Fallback - check serial_running status
                elif hasattr(self, 'serial_running') and any(self.serial_running.values()):
                    # Find which port is currently running
                    for port_name, is_running in self.serial_running.items():
                        if is_running:
                            device_type = 'serial'
                            device_identifier = port_name
                            device_label = self.device_labels["serial"].get(device_identifier, f"Unlabeled Serial ({device_identifier})")
                            self.log_multi_serial(f"Device source: Serial (fallback) | Port: {device_identifier} | Label: {device_label}")
                            break
                
                # Priority 4: File import (manual)
                else:
                    device_type = 'file'
                    device_identifier = 'manual_import'
                    device_label = 'Manual File Import'
                    self.log_multi_serial(f"Device source: File | Label: {device_label}")
                
                # Validate we have all required info
                if not device_label or not device_type or not device_identifier:
                    device_label = 'Unknown Device'
                    device_type = 'unknown'
                    device_identifier = 'unknown'
                    self.log_multi_serial(f"⚠️ Warning: Could not determine device info, using defaults")

                # Call database function with explicit type casting
                cur.execute("""
                ...
                self.log_multi_serial(f"Device ID from database: {device_id}")

                # ===== 2. PARSE SAMPLE TIME =====
                sample_time = self.patient.get('sample_time', '').strip()
                ...
                total_results = len(self.results) if data_format in ["HL7", "CUSTOM_HL7", "URIT_8030"] else 0
                ...
                if data_format in ["HL7", "CUSTOM_HL7", "URIT_8030"] and len(self.results) > 0:
                ...
                # ===== SUCCESS MESSAGE =====
                success_msg = f"{data_format} saved: Record #{record_id} | Device: {device_label}"
                if data_format in ["HL7", "CUSTOM_HL7", "URIT_8030"]:
                    success_msg += f" | Tests: {len(self.results)}"
                ...
                error_msg = f"Database function not found: {str(e)}\n\nPlease run the SQL script to create get_or_create_device() function."
                print(error_msg)
...
# 7. ===SETTING SOCKET SERVER METHODS===
    ...
    def handle_client(self, client_socket, address):
        """Handle individual client connections with Custom HL7, ASTM, and standard HL7 detection"""
        self.current_socket_ip = address[0]
        self.auto_register_socket_device(address[0])

        device_label = self.device_labels["socket"].get(address[0], "Unlabeled Device")
        self.current_device_label = device_label
        self.log_socket_message(f"Connection from {address[0]} ({device_label})")
        try:
            with client_socket:
                # ✅ KIRIM ACK SAAT KONEKSI PERTAMA KALI
                try:
                    initial_ack = "<ACK>\n"
                    client_socket.send(initial_ack.encode('utf-8'))
                    self.root.after(0, lambda: self.log_socket_message(
                        f"Initial ACK sent to {address[0]}:{address[1]}"
                    ))
                except Exception as e:
                    self.root.after(0, lambda: self.log_socket_message(
                        f"Failed to send initial ACK to {address[0]}:{address[1]}: {str(e)}"
                    ))
                ...
                while self.socket_running:
                    ...
                    # Log received chunk
                    self.root.after(0, lambda: self.log_socket_message(
                        f"Chunk received from {address[0]}:{address[1]} ({len(received_data)} bytes)"
                    ))
                    
                    # ===== CHECK 1: Custom HL7 =====
                    is_custom_hl7 = "#VTM" in data_buffer or ("#CR" in data_buffer and "OBX" in data_buffer)
                    
                    if is_custom_hl7:
                        has_end_marker = "#FS" in data_buffer or "#FS#CR" in data_buffer
                        is_large = len(data_buffer) > 500
                        
                        if has_end_marker or is_large:
                            complete_data = data_buffer.strip()
                            data_buffer = ""
                            
                            self.root.after(0, lambda: self.log_socket_message(
                                f" Custom HL7 data complete ({len(complete_data)} bytes)"
                            ))
                            ...
                            # ✅ KIRIM ACK SETELAH PARSE
                            try:
                                ack = "<ACK>\n"
                                client_socket.send(ack.encode('utf-8'))
                                self.root.after(0, lambda: self.log_socket_message(
                                    f"ACK sent to {address[0]}:{address[1]} (Custom HL7)"
                                ))
                            except Exception as e:
                                self.root.after(0, lambda: self.log_socket_message(
                    ...
                    # ===== CHECK 2: ASTM Format =====
                    is_astm = "STXA" in data_buffer
                    
                    if is_astm and len(data_buffer) > 100:
                        has_sub = "#SUB" in data_buffer or "SUB" in data_buffer
                        is_large = len(data_buffer) > 500
                        
                        if has_sub or is_large:
                            complete_data = data_buffer.strip()
                            data_buffer = ""
                            
                            self.root.after(0, lambda: self.log_socket_message(
                                f"ASTM data complete ({len(complete_data)} bytes)"
                            ))
                            ...
                            try:
                                ...
                                client_socket.send(ack.encode('utf-8'))
                                self.root.after(0, lambda: self.log_socket_message(
                                    f"ACK sent to {address[0]}:{address[1]} (ASTM)"
                                ))
                            except Exception as e:
                                self.root.after(0, lambda: self.log_socket_message(
                                    f"Failed to send ACK: {str(e)}"
                    ...
                    # ===== CHECK 3: Standard HL7 =====
                    if "MSH" in data_buffer and ("OBX" in data_buffer or len(data_buffer) > 200):
                        complete_data = data_buffer.strip()
                        data_buffer = ""
                        
                        self.root.after(0, lambda: self.log_socket_message(
                            f"Data received from {address[0]}:{address[1]} ({len(complete_data)} bytes)"
                        ))
                        ...
                        try:
                            ...
                            client_socket.send(ack.encode('utf-8'))
                            self.root.after(0, lambda: self.log_socket_message(
                                f"ACK sent to {address[0]}:{address[1]} (HL7)"
                            ))
                        except Exception as e:
                            self.root.after(0, lambda: self.log_socket_message(
                                f"Failed to send ACK: {str(e)}"
    ...
    def parse_socket_data(self):
        """Parse received socket HL7/ASTM data"""
        try:
            ...
            # Auto-detect and parse
            data_format = self.detect_data_format(socket_data)
            self.patient, self.results = self.parse_data_auto(socket_data)
            
            # ✅ SIMPAN data_format
            self.current_data_format = data_format
            
            # ✅ NEW: Tentukan device source
            ...
            device_source = f"{device_label} ({device_ip})"
            ...
            # Update results display dengan device info
            self.display_patient_info_with_device(device_source, format_display)
            ...
            # Enable save button
            self.save_socket_btn.configure(state=tk.NORMAL)
            self.log_socket_message(f"Socket data parsed ({format_display}) - Patient data ready")

            # Auto-save
            try:
                threading.Thread(target=self.save_to_database, daemon=True).start()
                self.log_socket_message("Auto-save started for socket data")
            except Exception as e:
                self.log_socket_message(f"Auto-save error (socket): {e}")
    ...
    def auto_parse_and_save(self, raw_data):
        ...
        try:
            ...
            if data_format == "HL7":
                ...
                self.log_serial_message("⚠️ Unknown data format, skipping parse.")
                return
...
# 8. ===SETTING RESULTS TAB MENU
    ...
    def display_patient_info_with_device(self, device_source, data_format):
        """Display patient info with device source information"""
        self.patient_info.delete(1.0, tk.END)
        
        # ✅ NEW: Device Source di baris pertama
        patient_text = f"Device Source: {device_source}\n"
        # patient_text += f"Data Format: {data_format}\n"
        # patient_text += f"{'='*60}\n"
        
        # Patient ID
        patient_text += f"Patient ID: {self.patient.get('patient_id', 'N/A')}\n"
        ...
        elif 'test_date' in self.patient and 'test_time' in self.patient:
            # ASTM format
            test_date = self.patient.get('test_date', '').strip()
            test_time = self.patient.get('test_time', '').strip()
            if test_date or test_time:
                sample_time = f"{test_date} {test_time}".strip()
                patient_text += f"Sample Time: {sample_time}\n"
                self.patient['sample_time'] = sample_time
        
        # Total Results (only for specific formats)
        current_format = getattr(self, 'current_data_format', 'HL7')
        if current_format in ["HL7", "CUSTOM_HL7", "URIT_8030"]:
            patient_text += f"Total Results: {len(self.results)}"
...
# 9. ===SETTING KONEKSI DAN LOGIKA PORT===
    ...
    def connect_single_port(self, port_name):
        ...
        def run_serial():
            try:
                ...
                # ✅ SET current_serial_port SEBELUM auto-register
                self.current_serial_port = port_name

                # Otomatis tambahkan port baru ke daftar Device Labels
                ...
                # ✅ Log untuk debugging
                self.log_multi_serial(f"🔗 Setting device info: Port={port_name}, Label={device_label}")
                ...
                try:
                    ...
                    self.root.after(0, lambda: self.log_multi_serial(
                        f"⚠️ Failed to send initial ACK to {port_name}: {str(e)}"
                    ))
                    ...
                    try:
                        if ser.in_waiting > 0:
                            ...
                            if is_complete:
                                ...
                                # ✅ UPDATE current_serial_port sebelum parse
                                self.current_serial_port = port_name
                                ...
                                self.root.after(0, lambda fmt=format_name, size=len(received_data), pn=port_name: 
                                    self.log_multi_serial(f"📥 [{pn}] Received {fmt} data ({size} bytes)")
                                )
                                ...
                                # ✅ CRITICAL: Set current_serial_port before parsing
                                self.root.after(0, lambda pn=port_name: setattr(self, 'current_serial_port', pn))
                                
                                # Auto parse and save
                                self.root.after(0, lambda data=received_data, pn=port_name: 
                                    self.auto_parse_and_save_serial(data, pn))
                                ...
                                try:
                                    ...
                                    ser.write(ack)
                                    ser.flush()
                                    self.root.after(0, lambda pn=port_name, fmt=format_name: 
                                        self.log_multi_serial(f"📤 ACK sent to {pn} ({fmt})")
                                    )
                                except Exception as e:
                                    self.root.after(0, lambda pn=port_name: 
                                        self.log_multi_serial(f"⚠️ [{pn}] Failed to send ACK: {str(e)}")
                                    )
    ...
    def disconnect_single_port(self, port_name):
        ...
        if not self.serial_running.get(port_name, False):
            self.log_multi_serial(f"ℹ️ [{port_name}] Already disconnected")
            return
        ...
        self.log_multi_serial(f"⏹️ [{port_name}] Disconnected")
        self.update_ports_display()
    ...
    def remove_selected_port(self):
        ...
        if messagebox.askyesno("Confirm", f"Remove port {port_name} from configuration?"):
            ...
            self.update_ports_display()
            self.log_multi_serial(f"🗑️ Port {port_name} removed from configuration")
    ...
    def disconnect_all_ports(self):
        ...
        self.log_multi_serial("⏹️ All ports disconnected")

    def clear_all_serial_data(self):
        """Clear all serial data"""
        self.clear_results_tab()
        self.log_multi_serial("🗑️ All serial data cleared")
    ...
    def auto_parse_and_save_serial(self, raw_data, port_name):
        """
        Automatically parse and save serial data with universal format support
        """
        try:
            # ✅ SET current_serial_port at the start
            self.current_serial_port = port_name
            ...
            # Detect format
            data_format = self.detect_data_format(raw_data)
            self.log_multi_serial(f"[{port_name}] Detected format: {data_format}")
            
            # Parse based on format
            if data_format == "BC1800":
                self.log_multi_serial(f"[{port_name}] Parsing BC-1800 format...")
                self.patient, self.results = self.parse_bc1800(raw_data)
                self.current_data_format = "BC1800"
                
            elif data_format == "URIT_8030":
                self.log_multi_serial(f"[{port_name}] Parsing URIT-8030 format...")
                self.patient, self.results = self.parse_urit_8030(raw_data)
                self.current_data_format = "URIT_8030"
                
            elif data_format == "BC5300_HL7":
                self.log_multi_serial(f"[{port_name}] Parsing BC-5300 format...")
                self.patient, self.results = self.parse_bc5300_hl7(raw_data)
                self.current_data_format = "BC5300_HL7"
                
            elif data_format == "CUSTOM_HL7":
                self.log_multi_serial(f"[{port_name}] Parsing Custom HL7 format...")
                self.patient, self.results = self.parse_custom_hl7(raw_data)
                self.current_data_format = "CUSTOM_HL7"
                
            elif data_format == "ASTM":
                self.log_multi_serial(f"[{port_name}] Parsing ASTM format...")
                self.patient, self.results = self.parse_astm_1394(raw_data)
                self.current_data_format = "ASTM"
                
            elif data_format == "HL7":
                self.log_multi_serial(f"[{port_name}] Parsing standard HL7 format...")
                self.patient, self.results = self.parse_hl7(raw_data)
                self.current_data_format = "HL7"
                
            else:
                # Unknown format - try fallback chain
                self.log_multi_serial(f"⚠️ [{port_name}] Unknown format, trying fallback parsers...")
                
                try:
                    self.patient, self.results = self.parse_bc1800(raw_data)
                    self.current_data_format = "BC1800"
                    self.log_multi_serial(f"✅ [{port_name}] Parsed as BC-1800 (fallback)")
                except:
                    try:
                        self.patient, self.results = self.parse_urit_8030(raw_data)
                        self.current_data_format = "URIT_8030"
                        self.log_multi_serial(f"✅ [{port_name}] Parsed as URIT-8030 (fallback)")
                    except:
                        try:
                            self.patient, self.results = self.parse_custom_hl7(raw_data)
                            self.current_data_format = "CUSTOM_HL7"
                            self.log_multi_serial(f"✅ [{port_name}] Parsed as Custom HL7 (fallback)")
                        except:
                            try:
                                self.patient, self.results = self.parse_hl7(raw_data)
                                self.current_data_format = "HL7"
                                self.log_multi_serial(f"✅ [{port_name}] Parsed as HL7 (fallback)")
                            except Exception as e:
                                self.log_multi_serial(f"❌ [{port_name}] All parsers failed: {str(e)}")
                                return
            
            # NEW: Tentukan device source untuk serial
            device_label = self.device_labels["serial"].get(port_name, f"Unlabeled Serial ({port_name})")
            device_source = f"{device_label} ({port_name})"
            ...
            format_display = {
                ...
                "HL7": "HL7",
                "ASTM": "ASTM"
            }.get(self.current_data_format, self.current_data_format)
            
            # Update Results display dengan device info
            self.root.after(0, lambda: self.display_patient_info_with_device(device_source, format_display))
            self.root.after(0, lambda: self.update_results_tree())
            
            # ✅ VERIFY device info before saving
            self.log_multi_serial(f"🔍 Before save: Port={self.current_serial_port}, Label={device_label}")
            ...
            # Success message
            patient_id = self.patient.get('patient_id', 'Unknown')
            sample_time = self.patient.get('sample_time', 'N/A')
            results_count = len(self.results)
            
            self.log_multi_serial(
                f"✅ [{port_name}] {format_display} data processed | "
                f"Patient: {patient_id} | Time: {sample_time} | Tests: {results_count}"
            )
            
        except Exception as e:
            import traceback
            error_detail = traceback.format_exc()
            self.log_multi_serial(f"❌ [{port_name}] Parse/Save error:\n{error_detail}")
