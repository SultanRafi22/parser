...
# ===== BATAS PENGATURAN UNTUK TRAY ICON ======
    ...
    def load_device_labels(self):
        """Load saved device labels from JSON file - ENHANCED VERSION"""
        ...
        if os.path.exists(self.config_file):
            try:
                ...
                if 'last_connected_serials' in config:
                    ...
                    print(f"Last session: {len(self.last_connected_serials)} serial ports were connected")  # ← GUNAKAN print()
    ...
    def load_current_customer(self):
        """Load current customer data from JSON file"""
    ...
    def save_current_customer(self):
        """Save current customer data to JSON file"""
        try:
...
# 3. ===SETTING TAB MENU TAMPILAN GUI===
    ...
    def create_socket_tab(self):
        ...
        self.socket_frame.grid_rowconfigure(1, weight=1)  # Log (expandable) - INI YANG BERUBAH
        self.socket_frame.grid_rowconfigure(2, weight=0)  # Buttons - INI YANG BERUBAH (dulu row 3)
        ...
        socket_process_frame.grid(row=2, column=0, padx=10, pady=5, sticky='ew')  # UBAH DARI row=3 ke row=2
    ...
    def create_customer_tab(self):
        """Create Customer information tab - MODIFIED with TRIPLE PanedWindow for fully resizable sections"""
        # Configure grid weights
        ...
        # Treeview - 4 kolom yang ditampilkan (ID dan Created disembunyikan)
    ...
    def format_current_customer_display(self):
        """Format current customer info for display"""
        if not self.current_customer.get('customer_code'):
            return "⚠️ No customer selected - Please select a customer to associate with test results"
    ...
    def set_current_customer(self):
        """Set form data as current active customer"""
        customer_code = self.customer_code_entry.get().strip()
        customer_name = self.customer_name_entry.get().strip()
        sales_code = self.sales_code_entry.get().strip()
        sales_name = self.sales_name_entry.get().strip()
    ...
    def populate_customer_tree(self, customers):
        """Populate customer treeview - ID dan Created Date disembunyikan dari tampilan"""
        # Clear existing
        for item in self.customer_tree.get_children():
            self.customer_tree.delete(item)
        
        # Add customers (store ID in item tags for internal use)
        for customer in customers:
            customer_id, code, name, sales_code, sales_name, created = customer
            
            # Insert HANYA 4 kolom yang visible, tapi simpan customer_id di tags
    ...
    def load_customer_from_tree(self, event):
        """Load customer data when double-clicked"""
        self.edit_selected_customer()

    def edit_selected_customer(self):
        """Edit selected customer from tree"""
        ...
        # FIX: Index berubah karena ID dan Created dihapus dari values
        # Urutan baru: [0]=Code, [1]=Name, [2]=Sales Code, [3]=Sales Name
        
        # Load to form
        self.customer_code_entry.delete(0, tk.END)
        self.customer_code_entry.insert(0, values[0])  # Index 0 = Customer Code
        
        self.customer_name_entry.delete(0, tk.END)
        self.customer_name_entry.insert(0, values[1])  # Index 1 = Customer Name
        
        self.sales_code_entry.delete(0, tk.END)
        self.sales_code_entry.insert(0, values[2] if values[2] != '-' else '')  # Index 2 = Sales Code
        
        self.sales_name_entry.delete(0, tk.END)
        self.sales_name_entry.insert(0, values[3] if values[3] != '-' else '')  # Index 3 = Sales Name

         # Focus on customer name for editing
        self.customer_name_entry.focus_set()
    ...
    def use_selected_customer(self):
        """Use selected customer as current active customer"""
        selected = self.customer_tree.selection()
        if not selected:
            messagebox.showwarning("Warning", "Please select a customer first!")
            return
        ...
        # FIX: Index berubah - [0]=Code, [1]=Name, [2]=Sales Code, [3]=Sales Name
        self.current_customer = {
    ...
    def delete_selected_customer(self):
        """Delete (soft delete) selected customer"""
        ...
        # FIX: Ambil customer_id dari tags, bukan dari values
        customer_id = int(tags[0]) if tags else None
        customer_code = values[0]  # Index 0 = Customer Code
        customer_name = values[1]  # Index 1 = Customer Name
...
# 5. ===SETTING LOGIKA PARSE===
    def parse_hl7(self, hl7_text):
        ...
        1. BC-5300: Patient ID dari OBR field 4 sub-component, NO results parsing
        2. URIT (8030/8031): Patient ID dari OBR field 3, WITH results parsing
        3. Standard HL7 (BS-120, BS-200, etc): Patient ID dari PID → OBR field 2 → OBR field 3, WITH results parsing
        """
...
# 6. ===SETTING MENU KONEKSI DATABASE===
    ...
    def save_to_database_with_context(self, patient, results, data_format, 
        ...
        """
        Save data to database with explicit device context including serial number and device type
        MODIFIED: Save '-' for all value fields
        """
        def save_data():
            try:
                self.update_config()
...
# 8. ===SETTING RESULTS TAB MENU
    ...
    def update_results_display_with_context(self, patient, results, data_format, device_source):
        ...
        MODIFIED: Display '-' for all value fields
        """
        try:
            ...
            if data_format in ["HL7", "Custom HL7", "URIT-8030"]:
                ...
                # MODIFIED: No color coding since all flags are '-'
                # Removed abnormal flag coloring
    ...
    def update_results_tree(self):
        """Update results tree view - MODIFIED to display '-' for all value fields"""
        # Clear results tree
        for item in self.results_tree.get_children():
            self.results_tree.delete(item)
        ...
        if data_format in ["HL7", "CUSTOM_HL7", "URIT_8030"]:
            ...
            # ⭐ MODIFIED: No color coding since all values are '-'
            # Removed abnormal flag coloring logic
...
# 11. ===SETTING API INTEGRATION METHODS===
    ...
    def generate_json_payload(self):
        ...
        MODIFIED: Send '-' for all value fields in API payload"""
